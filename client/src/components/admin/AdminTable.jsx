  ...
  return (
    <div>
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        >
          {columns.map((col) => (
            <div key={col.key} className={`text-xs font-medium text-gray-500 uppercase tracking-wider ${col.align === "center" ? "text-center" : col.align === "right" ? "text-right" : ""}`}>
              {col.label}
            </div>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          data.map((row, i) => (
            ...
            >
              {columns.map((col) => (
                <div key={col.key} className={col.align === "center" ? "text-center" : col.align === "right" ? "text-right" : ""}>
                  {col.render ? col.render(row) : <span className="text-gray-400 text-sm">{row[col.key]}</span>}
                </div>
              ))}
            </motion.div>
          ))
